...
const suggestVenue = async (req, res) => {
  ...
  // ✅ validation
  if (!name || !district_id || !address) {
    return res.status(400).json({ message: "Missing required fields" });
  }

  // ✅ fix boolean issues from frontend
  const outletAvailability =
    outlet_availability === true || outlet_availability === "true";
...
// ==============================
// ADMIN: REJECT venue (soft delete)
// ==============================
const rejectVenue = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "Venue rejected and deleted successfully"
    });
